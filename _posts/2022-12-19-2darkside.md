...
### Safety Measures
(might be generic but it is what it is)
- Follow best security practices
	- Maintain lockout policies (to counter bruteforce logins),
	- Use tough and unique passwords for login, > 15 characters (try passphrases instead of words), change it every ~90 days
	- Deploy Multi-Factor Authentication (MFA), 
	- Grant minimum priviliges to users according to their function; revoke prermissions after objective accomplished
- Use VPN for remote accessing network. Turn off RDP if not used. Change the RDP port to a non-standard port
- Regularly patch the apps, software and OS 
- Regular Backup
